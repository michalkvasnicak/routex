...
test('Route.constructor() throws if path is not an string', () => {
...
test('Route.constructor() throws if base path is not an string', () => {
    [true, 1, 1.0, null].forEach((path) => {
        const type = typeof path;
...
test('Route.constructor() throws if route children is not an array or function', () => {
...
test('Route.constructor() throws if route onEnter handler is not an function', () => {
    expect(() => new Route('/', '', [], 'a')).to.throw(
        `Invariant Violation: Route handler \`onEnter\` should be a function, string given.`
    );
});

test('Route.constructor() throws if route onLeave handler is not an function', () => {
    expect(() => new Route('/', '', [], () => {}, 'a')).to.throw(
        `Invariant Violation: Route handler \`onLeave\` should be a function, string given.`
    );
});

test('Route.match() rejects if async route definition does not return Promise or array', (done) => {
...
test('Route.match() rejects if route contains multiple variables of the same name', () => {
    const asyncRoutes = () => {
...
test('Route.match() resolves simple route without children', () => {
    const onEnter = () => {};
    const onLeave = () => {};
...
test('Route.match() resolves route with async children', () => {
    const asyncRoutes = () => {
        return Promise.resolve([
            {
                path: '/',
...
test('Route.match() resolves route with sync children', () => {
    const routes = [
        {
            path: '/',
            component: 'b'
...
test('Route.match() asynchronously tries to match a route and rejects with an error if not found', (done) => {
    const asyncRoutes = () => {
        return Promise.resolve([
            {
                path: '/',
