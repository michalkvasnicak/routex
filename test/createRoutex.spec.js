...
test('createRoutex() exposes redux public API + router instance on redux store', () => {
...
test('createRoutex() runs listeners on failed transition dispatch and sets state in reducer', (done) => {
    routex.router.run = spy(routex.router.run);
...
test('createRoutex() runs only not found listeners on transition dispatch to non existing route', (done) => {
    routex.router.run = spy(routex.router.run);
